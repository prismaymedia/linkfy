...
export class SpotifyService {
    ...
    async searchSpotifyTrack(trackName: string, artistName: string): Promise<SpotifyTrackInfo | null> {
        try {
            this.logger.log(`🔍 Searching Spotify for: "${trackName}" by "${artistName}"`);
            ...
            for (const query of searchQueries) {
                ...
                this.logger.log(`📋 Found ${items.length} results for query: "${query}"`);

                for (const track of items) {
                    const spotifyTrackName = track.name.toLowerCase();
                    const spotifyArtistNames = track.artists.map((a: any) => a.name.toLowerCase());

                    this.logger.log(`🔍 Checking: "${track.name}" by ${track.artists.map((a: any) => a.name).join(', ')}`);
                    ...
                    const artistNameSimilar = spotifyArtistNames.some(
                        (artist: string) =>
                            ...
                            this.calculateArtistNameSimilarity(artistName.toLowerCase(), artist)
                    );

                    if (trackNameSimilar && artistNameSimilar) {
                        this.logger.log(`✅ Match found: "${track.name}" by ${track.artists.map((a: any) => a.name).join(', ')}`);
            ...
            this.logger.log(`❌ No Spotify tracks found for "${trackName}" by "${artistName}"`);
            return null;
        } catch (error) {
    ...
    private calculateTrackNameSimilarity(youtubeTitle: string, spotifyTitle: string): boolean {
        ...
        if (cleanYoutube.includes(cleanSpotify) || cleanSpotify.includes(cleanYoutube)) {
        ...
        const youtubeWords = cleanYoutube.split(/\s+/).filter(word => word.length > 2);
        const spotifyWords = cleanSpotify.split(/\s+/).filter(word => word.length > 2);
        
        if (youtubeWords.length === 0 || spotifyWords.length === 0) return false;
        
        const significantWords = ['the', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for', 'of', 'with', 'by'];
        const filteredYoutubeWords = youtubeWords.filter(word => !significantWords.includes(word));
        const filteredSpotifyWords = spotifyWords.filter(word => !significantWords.includes(word));
        
        if (filteredYoutubeWords.length === 0 || filteredSpotifyWords.length === 0) {
            const commonWords = youtubeWords.filter(word => 
                spotifyWords.some(spotifyWord => 
                    this.levenshteinDistance(word, spotifyWord) <= 1
                )
            );
            
            const similarity = commonWords.length / Math.max(youtubeWords.length, spotifyWords.length);
            return similarity >= 0.8;
        }
        
        const commonWords = filteredYoutubeWords.filter(word => 
            filteredSpotifyWords.some(spotifyWord => 
                this.levenshteinDistance(word, spotifyWord) <= 1
            )
        );
        
        const similarity = commonWords.length / Math.max(filteredYoutubeWords.length, filteredSpotifyWords.length);
        return similarity >= 0.8;
    }

    private calculateArtistNameSimilarity(youtubeArtist: string, spotifyArtist: string): boolean {
        const cleanYoutube = this.cleanArtistName(youtubeArtist);
        const cleanSpotify = this.cleanArtistName(spotifyArtist);
        
        if (cleanYoutube === cleanSpotify) return true;
        
        if (cleanYoutube.includes(cleanSpotify) || cleanSpotify.includes(cleanYoutube)) return true;
    ...
    private cleanTitle(title: string): string {
        return title
            .replace(/[^\w\s]/g, ' ')
            .replace(/\s+/g, ' ')
            .replace(/\b(official|audio|lyric|hd|remix|cover|live|acoustic|instrumental)\b/gi, '')
            .trim()
            .toLowerCase();
    }
